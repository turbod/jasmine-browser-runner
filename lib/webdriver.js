function buildWebdriver(browserInfo, webdriverBuilder) {
  ...
  let browserName;
  let customBrowserCaps = [];

  if (typeof browserInfo === 'string') {
    browserName = browserInfo;
  } else if (browserInfo) {
    browserName = browserInfo.name;
    if (browserInfo.caps && Array.isArray(browserInfo.caps)) {
      customBrowserCaps = browserInfo.caps;
    }

    if (browserInfo.caps && typeof browserInfo.caps === 'string') {
      customBrowserCaps = [browserInfo.caps];
    }
  }
